...
export class CompaniesApi extends runtime.BaseAPI {
     ...
     */
    async getCompaniesRaw(requestParameters: GetCompaniesRequest): Promise<runtime.ApiResponse<Array<Company>>> {
     ...
     * Returns all companies from the system that the user has access to
     */
    async getCompanies(requestParameters: GetCompaniesRequest): Promise<Array<Company>> {
    ...
    async getCompanyRaw(requestParameters: GetCompanyRequest): Promise<runtime.ApiResponse<Company>> {
        if (requestParameters.companySlug === null || requestParameters.companySlug === undefined) {
            throw new runtime.RequiredError('companySlug','Required parameter requestParameters.companySlug was null or undefined when calling getCompany.');
        }
        ...
        const response = await this.request({
            path: `/companies/{companySlug}`.replace(`{${"companySlug"}}`, encodeURIComponent(String(requestParameters.companySlug))),
            method: 'GET',
            headers: headerParameters,
            query: queryParameters,
        });
...
/**
    ...
    * @enum {string}
    */
export enum GetCompaniesSortByEnum {
